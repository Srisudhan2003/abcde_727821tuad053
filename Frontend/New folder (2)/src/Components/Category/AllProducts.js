  ...
  useEffect(() => {
    const fetchProducts = async (categoryId) => {
      try {
        ...
        if (!response.ok) {
          throw new Error(`Login to view Category`);
        }
        ...
        for (const product of data) {
          const reviewsResponse = await fetch(`http://localhost:8080/api/productreviews/byProduct/${product.productId}`, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          });
  ...
  if (loading) {
    return (
      <div className='flex items-center justify-center h-screen'>
        <div className="inline-block h-8 w-8 animate-spin rounded-full border-4 border-solid border-current border-r-transparent align-[-0.125em] motion-reduce:animate-[spin_1.5s_linear_infinite] justify-center text-center items-center" role="status">
              ...
              <div key={product.productId} className="group relative p-4 border-r border-b border-gray-200 sm:p-6">
                <div className="rounded-lg overflow-hidden bg-gray-200 aspect-w-1 aspect-h-1 group-hover:opacity-75">
                  <img
                    ...
                    className="w-full h-full object-center object-cover transition duration-500 group-hover:scale-105 sm:h-72 cursor-pointer"
                ...
                <div className="pt-10 pb-4 text-center">
                  ...
                  <div className="mt-3 flex flex-col items-center">
                    <p className="sr-only">{productRatings[product.productId]} out of 5 stars</p>
                    <div className="flex items-center">
                      {[0, 1, 2, 3, 4].map((rating) => (
                        <StarIcon
                          key={rating}
                          className={classNames(
                            productRatings[product.productId] > rating ? 'text-yellow-400' : 'text-gray-200',
